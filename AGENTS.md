...
pnpm lint:copy      # copy law gate: em dashes, banned words, chatbot tone
pnpm gate           # check + lint:copy + build, real exit code
...
4. **Layout follows the artboard.** Desktop 8a and mobile 8b are both first-class; mobile is "edited, not stacked" (some elements are dropped or shortened on mobile, per `docs/DESIGN-SPEC.md`). Tokens only; no new colours, radii, or fonts. Lime is never text on a light background.
...
src/
  ...
  pages/                index, field-notes/{index,[slug]}, principles, about, archive/{index,[slug]}, methodology, contact, 404, rss.xml.ts
...
scripts/                lint-copy.mjs import-legacy.mjs
