...
- One breakpoint does most of the work: `--bp-md: 768px`. Below it, the mobile artboard applies; above it, the desktop artboard applies. Use `clamp()` for type between the two reference sizes where a value differs (values listed per section).
- Desktop page gutter 96px, mobile 20px. Content max width 1440 (centred). Between 768 and 1440 the 96px gutter and the 160px label column stay; the content column shrinks.
...
### Nav
- Desktop: height 72, cream, bottom rule. Left: "Rob Saric" wordmark link (17px/700, `--forest-500`, -0.015em). Right: links "Field notes", "Principles", "About" (16px/500 ink, 44px tall hit area, gap 32) then DS Button dark lg "Caretrics" with `arrow-up-right`, external to `SITE.caretricsUrl`.
...
- Desktop: padding 96; label grid; column gap 44.
  ...
  - Then a two-column row (gap 56, top rule, padding-top 40): "What Caretrics does" H3 22px/700 + body 18px + trust line 15px muted; "What it does not do" H3 + body + DS Button dark 48 "See Caretrics" (padding-top 6).
  - Then Togetheren row (top rule, padding-top 28, flex baseline space-between gap 40): left "Selected work through Togetheren" 18px/600 + 17px muted description (max 62ch); right link 17px/600 teal "See selected work →" (external `SITE.togetherenUrl`).
- Mobile: padding 44 20 gap 18: eyebrow, H2 27px, body 17px (shorter, no "Select a stage."), stage panel as an accordion (each row: 56px min button full width, padding 16 18, bg white/`--wash` when open, dot + name 19px/700 left, mono `+`/`−` right `aria-hidden`; open panel padding 4 18 20 gap 12: question 19px/600, signals 16px, note 15px muted; rows separated by bottom rule). Then combined paragraph ("It reads 12 months … clinically needs.") 17px, trust line 15px, DS Button full width. Then Togetheren block (top rule, padding-top 18, gap 6, link 44 tall).
...
- Desktop: padding 96; label grid; inner grid `360px 1fr` gap 64 `align-items: center`. Left photo 360×400 (rob-family, alt "Rob Saric outdoors", lazy). Right column gap 24: H2 "Where the standard came from."; body 18px max 52ch; serif quote 30px max 24ch; stat row (top rule, padding-top 20, flex wrap gap 40) three 17px items.
- Mobile: padding 44 20 gap 16: eyebrow, H2, photo full width height 280 cover, body 17px, quote 24px, stat column (gap 10, top rule padding-top 16) 16px items.
...
### Footer (forest-700)
- Desktop: padding 56 96, column gap 20: 20px/1.4 on-dark line (max 44ch) "I look for people building things that help others live better lives."; 16px on-dark-78 "Rob Saric · Founder of Caretrics · Ottawa."; link row (top rule on-dark-28, padding-top 16, flex wrap gap 28, each link 44 tall, 16px on-dark): LinkedIn, X, GitHub, "View source ↗", "Caretrics ↗" (weight 700), "Togetheren, selected work" (on-dark-78); then 15px on-dark-78 line "Earlier writing on systems and flow, 2009 to 2019, is in the archive." with "the archive" linked to `/archive/`.
...
- `/field-notes/[slug]/` — article layout: eyebrow (type · date · stage), H1 title, prose at 18px/1.6 max 68ch, `<article>`; footer nav back to `/field-notes/`. Prose styles: h2 26px, h3 20px, links teal underline, blockquote serif italic 24px with left rule, code mono 15px, images radius 12.
...
- `/methodology/` — H1 "What the number means", stub content from `docs/COPY.md` (marked TODO), `noindex` until the TODO is cleared.
