...
- Backgrounds in page order: cream (nav, hero, now strip) → forest-500 (01 Evidence) → cream (operator proof, unnumbered) → cream (02 Caretrics) → cream + rule (03 Principles) → cream + rule (04 Field notes) → cream + rule (05 About) → lime-100 (06 Write to me) → forest-700 (footer).
...
### DS Button (dark)
From the Caretrics DS `Button` (variant `dark`, size `lg`): height 44px, padding 0 20px, font 15px/600, radius 8px, background `--forest-500`, colour white, border 1px solid `--forest-500`, inline-flex, gap 6px, `white-space: nowrap`, transition 150ms. Optional trailing icon 14px (stroke 1.75): `arrow-right` (See Caretrics) or `arrow-up-right` (nav Caretrics, external). Hero and section CTAs render at 48px height (`hint-size` 48). Mobile CTAs are full width (`width: 100%`). Hover: background `--forest-700`; the `arrow-right` icon also nudges 2px right over `--dur-micro` `--ease-out` (2026-08-24, approved UI-tweaks canvas). The nudge is scoped to `arrow-right` only: sliding the `arrow-up-right` external glyph sideways reads as the wrong direction. Implement as `Button.astro` with `href` (renders `<a>`) or `type` (renders `<button>`), inline SVG icons (no icon library).
...
- Wayfinding and motion (2026-08-24, from the approved UI-tweaks canvas). Every nav link, desktop row and mobile panel, carries `aria-current="page"` when the path sits inside its section; a current link reads `--forest-500` at weight 600. The mobile toggle icon is three separate strokes that morph to a close X over `--dur-short` `--ease-out`: outer strokes `rotate(±45deg) translateY(∓5px)` about the icon centre, middle stroke fades. The open panel enters with `rs-menu-enter` (fade + 6px descent, `--dur-short`), and its links follow at 40ms stagger steps with `animation-fill-mode: both`; opacity lives only in the keyframes, per the `rs-panel-enter` contract in global.css, so reduced-motion users always see the links. Closing is instant on purpose: exits should be faster than entrances. Elevation: under `@supports (animation-timeline: scroll())` the bar gains `--shadow-nav` across the first 48px of document scroll, so the shadow exists only once content is actually beneath the bar; without support, and under reduced motion, the constant hairline is the whole treatment, exactly as before.
...
- Deviations from the artboard, 2026-08-27 (Rob's rulings, strings in `docs/COPY.md` Hero): the tagline line is REMOVED at both widths; the H1 measure widens 17ch to 19ch for the longer headline; the portrait gains the lime offset card (the `/about/` header treatment: lime-500 block offset -14/-14 desktop, -10/-10 mobile) and the hero elements enter with one staggered fade-rise (0.6s, opacity/transform only, dead under `prefers-reduced-motion`); CTA labels and all copy per `docs/COPY.md`.
...
- Desktop: padding 96; label grid; inner grid `400px 1fr` gap 72 `align-items: start`.
  - Left column (gap 24): H2 on-dark "The dashboard held the evidence. The work still had no owner."; body 18px on-dark; the metric block (cut 2026-08-19, **restored 2026-08-24**, docs/COPY.md V5): top rule `--on-dark-28`, padding-top 20, gap 10: metric `$127,000` (44px/700/-0.03em, `tabular-nums`, `aria-describedby` the note), note 17px on-dark, then mono 14px lime link "How I counted this →" to `/how-i-counted/` (44px tall).
...
- Mobile: padding 44 20, column gap 20: eyebrow, H2 27px on-dark, short body ("One anonymized signal, from detection through to a verified outcome."), then the metric block (padding-top 16, gap 8, value 34px/-0.028em per 8b, note 16px; 2026-08-24; deliberate deviation from artboard 8b, which draws the metric after the panel: proof reads before the interaction, and mobile is edited, not stacked), then panel (border only, no fill, padding 20, gap 18): signal block; step rail as 5 numbered buttons (`--rail-w: 3px`, min-height 44, `aria-label` = step label, mono 13px number only); content min-height 210; lime button full width; exit link 44px tall.
...
### Operator proof (cream, unnumbered, between 01 and 02; 2026-08-24)
- One named operator quote in the site's blockquote grammar. Desktop: padding 88 96 0 (02's own 96px top padding provides the space below); `<figure>` gap 16: blockquote max 62ch, padding-left 20, 3px `--forest-500` left rule, serif italic 24px/1.4 ink; figcaption 16px `--ink-muted`, padding-left 23 so it aligns with the quote text. `aria-label` "From a clinic owner". No heading, no number: 01 to 06 stay the homepage's table of contents.
- Mobile: padding 40 20 0; figure gap 12; quote 19px/1.45, padding-left 16; attribution 15px, padding-left 19.
- Data: `src/data/operator-proof.ts` (quote verbatim from caretrics.com, permission noted in the file). No JS.
...
- `/how-i-counted/` (2026-08-24) — the methods page, named for what it does rather than "methodology": it exists to be checked, not to assert rigour. Page-header pattern (H1 48px, intro 18px max 58ch with the note link), then a rule-separated list of eight H2 (22px/700) + one-paragraph (18px, max 58ch) sections (the eighth, "The $127,000.", added 2026-08-24 with the metric restoration), then the contact exit link. Same paddings as `/principles/`; mobile drops to 34px H1 and 17px body. No new tokens, no JS.
