...
- Deviations from the artboard, updated 2026-08-29 (strings in `docs/COPY.md` Hero): the tagline and the 17ch H1 measure are back to the artboard (the 08-27 removal and widening were superseded with the hero); what remains deviated is the portrait's lime offset card (the `/about/` header treatment: lime-500 block offset -14/-14 desktop, -10/-10 mobile) and the staggered fade-rise entrance (0.6s, opacity/transform only, dead under `prefers-reduced-motion`).
